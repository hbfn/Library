...
public class InterfaceUsuarioListar extends JFrame{
	...
	public InterfaceUsuarioListar() {
		...
		panelCadastrarUsuario.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "Lista de Usuários", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
		panelCadastrarUsuario.setBounds(44, 24, 923, 679);
		getContentPane().add(panelCadastrarUsuario);
		
		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(10, 81, 903, 552);
		...
		btnDeletar.setBounds(679, 644, 115, 24);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (table.getSelectedRow() >= 0) {
					...
					try {
						...
						itfUsuarioCadastrar.carregarUsuario(usuario);
						System.out.println(usuario);
					} catch (DaoException e1) {
						JOptionPane.showMessageDialog(null, "Usuário não Encontrado!");
					}
		...
		btnEditar.setBounds(559, 644, 115, 24);
